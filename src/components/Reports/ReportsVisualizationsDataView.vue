<template>
    ...
    <DataView v-else class="field flex flex-column" :value="reportVisualizations" data-key="visualizationId"
        :layout="layout">
        <template #header>
        ...
        <template #list="slotProps">
            <div class="p-card w-full p-2 m-1">
                <div class="p-card-title">{{ slotProps.data.visualizationName }}</div>
                <div class="p-card-body">
                    ...
                    <div class="flex justify-content-between mt-2">
                        ...
                        <small>ID: {{ slotProps.data.visualizationId }}</small>
                    </div>
                </div>
    ...
    <Dialog v-model:visible="displayDelete" :style="{ width: '450px' }" header="Confirm" :modal="true">
        <div class="confirmation-content flex align-content-center justify-content-center">
            ...
            <span>Are you sure you want to delete analyze <b>{{ selectedVisualization?.visualizationName }}</b>?</span>
        </div>
        <template #footer>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const submitDelete = async () => {
            ...
            const response = await deleteVisualization(selectedVisualization.value.visualizationId)
            ...
            closeDialog()
            getReportVisualizations(props.report.reportId)
        }
