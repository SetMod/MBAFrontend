<template>
    ...
    <div v-else>
        <UserOrganizationsDataTableVue v-if="organizationMembers" :organizations="organizations" :open-leave="openLeave"
...
const organizations = ref(props.organizationMembers ? organizationMembers : allOrganizations)
const selectedOrganization = ref<Organizations>()
const refreshTable = () => {
    if (isLoggedIn && user.value) getOrganizationMembers(user.value.userId)
    getOrganizations()
    getOrganizationsRoles()
}
const closeDialog = () => {
...
const submitJoin = async (selectedOrganization: Organizations) => {
    ...
    await getOrganizationMembers(user.value.userId)
}
