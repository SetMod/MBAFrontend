...
const emit = defineEmits({
    submit: (analyze: Analyzes) => {
        if (analyze instanceof Analyzes) return true
        console.log('Validation error of submit event params');
        return false
    }
})

const analyzeCreateState = reactive<Analyzes>(new Analyzes())
...
const selectedDatasource = computed(() => {
    return props.datasources.find(ds => ds.id == analyzeCreateState.datasourceId)
})
</script>
    ...
    <div class="p-card m-1 w-full">
        ...
        <div class="p-card-body  text-left">
            ...
            <div class="field flex justify-content-center">
                ...
                <Button label="Create" severity="success" :disabled="analyzeValidate.$invalid"
                    @click="() => emit('submit', analyzeCreateState)" />
